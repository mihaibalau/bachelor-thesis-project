...
use crate::service::{
    affiliate::{AffiliateService, ListAffiliatesParams},
    errors::{ServiceError, ServiceResult},
    transaction::TransactionService,
    user::{UserService, UserWithAccounts},
};
use crate::db::{
...
pub struct DashboardService {
    user_svc: Arc<UserService<UserRepo, AccountRepo>>,
    tx_svc: Arc<TransactionService<TransactionRepo, AccountRepo>>,
    affiliate_svc: Arc<AffiliateService<AffiliateRepo, AccountRepo, UserRepo>>,
}
...
impl DashboardService {
    ...
    #[tracing::instrument(skip(self), fields(user_id = %user_id.0))]
    pub async fn get_dashboard(&self, user_id: UserId) -> ServiceResult<DashboardData> {
        const PER_ACCOUNT_LIMIT: i64 = 500;
        const RECENT_ACTIVITY_LIMIT: usize = 5;

        let UserWithAccounts { accounts, .. } = self.user_svc.get_user_with_accounts(user_id).await?;
        ...
        let net_change = current_stats.total_incoming_cents - current_stats.total_outgoing_cents;
        ...
        let all_transactions = self
            .collect_user_transactions(user_id, &accounts, PER_ACCOUNT_LIMIT)
            .await?;
        ...
        recent.sort_by(|a, b| {
            b.recorded_on()
                .cmp(&a.recorded_on())
                .then_with(|| b.id().map(|i| i.0).cmp(&a.id().map(|i| i.0)))
        });

        let mut recent_activity = Vec::new();
        for tx in recent.into_iter().take(RECENT_ACTIVITY_LIMIT) {
            recent_activity.push(map_activity_item(&tx, &user_account_ids)?);
        }
    ...
    async fn collect_user_transactions(
        ...
        for account in accounts {
            let account_id = account
                .id()
                .ok_or_else(|| ServiceError::internal("account missing id"))?;
            let txs = self
                .tx_svc
...
) -> ServiceResult<DashboardActivityItem> {
    let id = tx
        .id()
    ...
    let (label, detail, category) = match tx.transaction_type() {
        TransactionType::Payment => {
            ...
            (
                merchant,
                detail,
                ui_category(tx.transaction_type(), &payment_category, description),
            )
        }
