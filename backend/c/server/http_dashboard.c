...
static json_t *activity_item_to_json(const Transaction *tx, const AccountId *owned, size_t owned_count) {
    ...
    json_object_set_new(o, "label", json_string(label));
    json_object_set_new(o, "description", json_string(detail));
    json_object_set_new(o, "recorded_on", json_string(recorded));
    json_object_set_new(o, "amount_cents", json_integer((json_int_t)amount_cents));
    json_object_set_new(o, "category", json_string(ui_category(t, pay_category, description)));
    json_object_set_new(o, "is_income", json_boolean(amount_cents > 0));
    return o;
}
...
// GET /api/dashboard — aggregate balances, spending, recent activity.
static enum MHD_Result handle_dashboard(AppState *state, struct MHD_Connection *conn) {
    ...
    if (!affiliate_service_list_affiliates_view(
            state->affiliate_svc, claims.sub, &aparams, &affiliates, &serr)) {
        free(owned);
        free_user_with_accounts(&dto);
        return http_send_service_error(conn, &serr);
    }
    ...
    if (!transaction_service_compute_user_statistics(
            state->tx_svc, claims.sub, 500, true, month_start, true, month_end, &cur_stats, &serr)) {
    ...
    if (!transaction_service_compute_user_statistics(
            state->tx_svc, claims.sub, 500, true, prev_start, true, prev_end, &prev_stats, &serr)) {
    ...
    for (size_t i = 0; i < dto.account_count; ++i) {
        ...
        if (!transaction_service_list_recent_for_user(
                state->tx_svc, claims.sub, aid, 100, 0, &txs, &count, &serr)) {
        ...
        for (size_t j = 0; j < count; ++j) {
            ...
            if (tx_count >= tx_cap) {
                ...
                if (!tmp) {
                    ...
                    return http_send_service_error(conn, &oom);
                }
                tx_entries = tmp;
                tx_cap = new_cap;
            }
    ...
    for (int day = 1; day <= plot_dim; ++day) {
        ...
        char date_str[11];
        snprintf(date_str, sizeof date_str, "%04d-%02d-%02d", cur_year, cur_month, day);

        char day_label[16];
        snprintf(day_label, sizeof day_label, "%s %d",
                 weekday_short(day_tm.tm_wday), day);

        json_t *pt = json_object();
        json_object_set_new(pt, "day_label", json_string(day_label));
        json_object_set_new(pt, "date", json_string(date_str));
        json_object_set_new(pt, "cumulative_spending_cents", json_integer((json_int_t)cumulative));
        json_array_append_new(daily, pt);
    }
    ...
    size_t added = 0;
    for (size_t i = 0; i < tx_count && added < RECENT_ACTIVITY_LIMIT; ++i) {
        json_array_append_new(recent, activity_item_to_json(tx_entries[i].tx, owned, dto.account_count));
        ++added;
    }
    ...
    json_object_set_new(spending, "total_spent_cents", json_integer((json_int_t)total_spent_cents));
    json_object_set_new(spending, "change_percent_vs_last_month", json_real(spending_change));
    json_object_set_new(spending, "daily_cumulative", daily);

    json_t *root = json_object();
    json_object_set_new(root, "total_balance_cents", json_integer((json_int_t)total_balance_cents));
    json_object_set_new(root, "balance_change_percent", json_real(balance_change_percent));
    json_object_set_new(root, "active_accounts_count", json_integer((json_int_t)dto.account_count));
    json_object_set_new(root, "affiliates_count", json_integer((json_int_t)affiliates.total));
    json_object_set_new(root, "transfers_total_cents", json_integer((json_int_t)transfers_total_cents));
